...
NO_dependency = True
plot_geometry = False
description = "two_pc_without_NO_Aminus_changed"
# %%-------------------------------------------CREATE NETWORK---------------------
"""Create network"""
...
nest.SetDefaults(
    "stdp_synapse_sinexp",
    {
        "A_minus": net_config["connections"]["A_minus"],
        "A_plus": net_config["connections"]["A_plus"],
        "Wmin": net_config["connections"]["Wmin"],
        "Wmax": net_config["connections"]["Wmax"],
        "vt": vt[0],
        "weight_recorder": WeightPFPC[0],
    },
)
PFPC_conn_param = {"model": "stdp_synapse_sinexp", "weight": Init_PFPC, "delay": 1.0}
vt_num = 0
PC_vt_dict = {}
source_ids = []
PC_ids = []
...
for i, PCi in enumerate(PC):
    nest.Connect(
        GR,
        [PCi],
        {"rule": "fixed_indegree", "indegree": int(0.8 * GR_num), "multapses": False},
        PFPC_conn_param,
    )
...
spikedetector_PC = nest.Create(
    "spike_detector",
    params={"withgid": True, "withtime": True, "to_file": True, "label": "purkinje"},
)
spikedetector_GR = nest.Create(
    "spike_detector",
    params={"withgid": True, "withtime": True, "to_file": True, "label": "granule"},
)
...
granule_density = net_config["geometry"]["granule_density"]
...
for n in range(num_subpop):
    ...
    if plot_geometry:
        fig.add_trace(
            go.Scatter3d(
                x=nos_coord_subpop[n][:, 0],
                y=nos_coord_subpop[n][:, 2],
                z=nos_coord_subpop[n][:, 1],
                mode="markers",
                marker=dict(size=3, color=color[n], opacity=0.5),
                name="nos sub pop " + str(n),
            )
        )
if plot_geometry:
    fig.add_trace(
        go.Scatter3d(
            x=PC_coord["PC_x"].to_numpy(),
            y=PC_coord["PC_z"].to_numpy(),
            z=PC_coord["PC_y"].to_numpy(),
            mode="markers",
            marker=dict(size=20, color=pc_color, opacity=0.8),
            name="PC",
        )
    )
    fig.show()
...
nest.SetStatus(
    PG_noise,
    params={"rate": noise_rate, "start": 0.0, "stop": 1.0 * trial_len * n_trial},
)
...
for index, mf in enumerate(PG_input):
    CS_spikes = homogeneous_poisson_for_nest(
        input_rate, (stop_CS - start_CS) / 1000, bin_size
    )
...
file_prefixes = ["granule","purkinje","sim_description","PFPC","two","NO_simulation"]
...
fig, axes = plt.subplots(
    n_trial, 1, figsize=(6, 20), constrained_layout=True, sharex=True, sharey=True
)
for trial in range(n_trial):
    start = trial * trial_len
    stop = stop_CS + trial * trial_len
...
PFPC_weight = pd.read_csv(
    csv_f, sep="\t", names=["PF_id", "PC_id", "t_ist", "weight"], usecols=[0, 1, 2, 3]
)
connection_dict = {}
data = {}
...
for i, PC_id in enumerate(PC):
    ...
    connection_dict[PC_id] = {
        "PF_id": PF_id,
        "weight_matrix": np.zeros((len(PF_id), int(n_trial * trial_len * 10))),
    }
        ...
        if weights.shape != 0:
            for id_weight in range(len(weights)):
                if (id_weight + 1) < len(weights):
                    data["weight_matrix"][i][
                        weights[id_weight] : weights[id_weight + 1]
                    ] = data["weight_matrix"][i][weights[id_weight]]
                elif id_weight + 1 == len(weights):
                    data["weight_matrix"][i][weights[id_weight] : -1] = data[
                        "weight_matrix"
                    ][i][weights[id_weight]]

for PC_id, data_pfpc in connection_dict.items():
    fig3, axes3 = plt.subplots(
        num_subpop, 2, figsize=(12, 10), constrained_layout=True, sharey=True
    )
