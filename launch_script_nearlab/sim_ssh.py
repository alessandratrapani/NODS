...
if type == 'test':
    ...
    A_plus = [10.25, 10.5, 10.75]
    A_minus = [15, 16, 17]

    for plus in A_plus:
        for minus in A_minus:
            int_part = str(int(plus))
            dec_part = str(plus).split('.')[1]
            plus_mod = int_part +'_'+dec_part

            for i in range(num_of_sim):
                # Create a unique results directory for each execution
                results_dir = f"/home/csartor1/code/NODS/results/{type}/plus{plus_mod}_minus{minus}/sim_{i}"
                os.makedirs(results_dir, exist_ok=True)
                ...
                # Run the shell script, passing the execution number as an argument
                with open(output_file, "w") as f:
                    subprocess.run(["bash", "run_grid_search.sh", type, str(plus_mod), str(minus), str(i)], stdout=f, stderr=f)
